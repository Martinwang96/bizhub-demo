...
import styles from './MobileOpsHeader.module.css';

interface MobileOpsHeaderProps {
  onRefresh: () => void;
  /** 全量刷新 in-flight，按钮显示 spinner 状态 */
  refreshing?: boolean;
...
/**
 * 移动端 Ops 页 sticky Header。
 * - 左：leading 槽（默认 pulse icon）+ 「系统日志告警」
 * - 右：「技能管理」(可选) + 「返回对话」ghost + 「刷新」ghost btn（带旋转图标）
 */
export default function MobileOpsHeader({ onRefresh, refreshing, leading, me }: MobileOpsHeaderProps) {
  return (
    <header className={styles.header}>
      <div className={styles.titleGroup}>
        ...
        <h1 className={styles.title}>系统日志告警</h1>
      </div>
